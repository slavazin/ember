...
and self-heals when a recycle has dropped it.
    ...
    { [ -e /corpus/README.md ] && [ -e /tenant-incident/corpus/README.md ]; } || {
      ...
      _tf_origin="${EMBER_ORIGIN:-$(printf %s "$_tf_skills" | jq -r --arg n corpus-write '([.[]|select(.name==$n).url]+[.[].url]|map(select(.!=null)))[0] // empty')}" &&
      _tf_origin="${_tf_origin:-https://github.com/slavazin/ember.git}" &&
      _tf_ref="${EMBER_CORPUS_REF:-$(printf %s "$_tf_skills" | jq -r --arg n corpus-write '([.[]|select(.name==$n).ref]+[.[].ref]|map(select(.!=null)))[0] // empty')}" &&
...
the guard's exec environment — the clone reads that ref, so the store it files against is the one the
frozen tag holds.
