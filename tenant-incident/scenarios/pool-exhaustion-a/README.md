# pool-exhaustion-a — orders timeout storm (Node · PostgreSQL)

## What it models

The `orders` service (Node.js/Express) holds a client-side connection pool of
max 5 to a healthy PostgreSQL. Each request checks out a pooled connection and
holds it about 1 s of datastore work (`pg_sleep(1)`). An nginx gateway fronts
the service with a 2 s upstream read timeout.
...
returns a storm of 504s — while Postgres stays healthy and emits no error,
because at most 5 connections ever reach it, far below `max_connections=100`.
The cause is visible only in pool-saturation metrics.

This is the "gateway timeout storm before any datastore error, pool metrics
checked late" failure class. Its recurrence on a different surface is
[/tenant-incident/scenarios/pool-exhaustion-b](/tenant-incident/scenarios/pool-exhaustion-b) (Python · Redis).

## The surface

- **Gateway:** nginx — `proxy_read_timeout 2s`, single upstream,
  `proxy_next_upstream off`. Host port `8080`.
- **Service:** `orders-svc` — Node.js/Express, `pg.Pool({ max: 5 })`, single
  process. Host port `3001` (direct access to `/debug/pool`).
- **Datastore:** PostgreSQL 16, `max_connections=100`.
...
curl -s localhost:3001/debug/pool   # the smoking gun: {"max":5,"in_use":5,"waiting":N}
docker compose logs gateway         # the 504 storm, with upstream_time at the 2 s timeout
docker compose logs orders-svc      # pool_wait_ms climbing; db_query_ms flat near 1000; no datastore error
...
- Scaling or restarting the datastore does not drain the queue: the datastore is
  healthy and idle, and the ceiling is the client pool.
- The service runs as a single process, so adding pooled connections is the only
  lever. Horizontal scaling would add a second pool and mask the cause, so it is
