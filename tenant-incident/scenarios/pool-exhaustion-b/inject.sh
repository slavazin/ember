...
echo "[inject] ensuring the core stack is up (gateway + catalog-svc + redis)..."
docker compose up -d --build redis catalog-svc gateway
...
for _ in $(seq 1 10); do
  code=$(curl -s -o /dev/null -w '%{http_code}' --max-time 6 "$GATEWAY/catalog/1" || echo 000)
  case "$code" in
    504) storm=$((storm + 1)) ;;
    200) ok=$((ok + 1)) ;;
    *)   other=$((other + 1)) ;;
...
echo "[inject] incident is live. Investigate via the gateway ($GATEWAY) and catalog-svc ($SVC)."
echo "[inject]   docker compose logs gateway      # the 504 storm"
echo "[inject]   docker compose logs catalog-svc  # pool_wait_ms climbing; redis_op_ms flat; no datastore error"
