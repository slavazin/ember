#!/usr/bin/env bash
# Score a diagnosis report for the notifications backlog-stall incident (poison
# message).
#
...
# A heading naming the queue/message/state, with a poison/head-of-line token
# inside that section.
STATE_MATCH=$(awk '
  { l = tolower($0) }
  l ~ /^#+[[:space:]].*(queue|message|state|backlog|head|record)/ { insec = 1; heading = $0; next }
  insec && l ~ /^#+[[:space:]]/ { insec = 0 }
  insec && l ~ /poison|malformed|deserial|does not (advance|clear)|not advancing|head.?of.?line|head of line|stuck|re-?deliver|quarantin|dead.?letter|dlq|bad (message|record)/ {
...
CAP_MATCH=$(awk '
  { l = tolower($0) }
  l ~ /(worker|redis|broker|capacity|cpu|scal|thread)/ &&
...
# state diagnosis from another producer fails only this gate. The canonical
