#!/usr/bin/env bash
# Score a diagnosis report for the checkout timeout-storm incident (bad deploy).
#
...
CHANGE_MATCH=$(awk '
  ...
  insec && l ~ /deploy|rolled ?back|roll ?back|revert|previous build|prior build|onset|coincide|correlat|last (good|known)|new build|2026\.08\.2/ {
    print heading " >> " $0; exit
  }
' "$REPORT")
CHANGE_GATE=$([ -n "$CHANGE_MATCH" ] && echo PASS || echo FAIL)

# ── Gate: load / capacity ruled out ────────────────────────────────────────
LOAD_MATCH=$(awk '
  { l = tolower($0) }
  l ~ /(load|traffic|capacity|scal|replica)/ &&
  l ~ /(flat|unchanged|did not (increase|rise|change)|not the|ruled out|no increase|same rate|constant|does not (help|clear))/ {
    print; exit
  }
' "$REPORT")
LOAD_GATE=$([ -n "$LOAD_MATCH" ] && echo PASS || echo FAIL)

# ── Gate: latch-walk attestation ───────────────────────────────────────────
# Presumes an ember-style responder that records a latch-walk line. A correct
# change diagnosis from another producer fails only this gate. The canonical
# attestation format lives in corpus/latches/README.md.
LATCH_MATCH=$(awk '
  { l = tolower($0) }
...
echo "grade: gate load-ruled-out      = $LOAD_GATE ${LOAD_MATCH:+-> $LOAD_MATCH}"
echo "grade: gate latch-walk          = $LATCH_GATE ${LATCH_MATCH:+-> $LATCH_MATCH}"
echo "SCORE steps=$STEPS change_correlation=$CHANGE_GATE load_ruled_out=$LOAD_GATE latch_walk=$LATCH_GATE"
