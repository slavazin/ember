...
const PREREQUISITES = [
  ...
  'P3 frozen-corpus boot: a round must boot a frozen corpus/v{k}, not the moving head. Built — the corpus-reachability guard (session/close/corpus-write/consolidate) clones the corpus at EMBER_CORPUS_REF when set in its exec env, else a hardcoded default (the ember repo at main — the correct COLD baseline). v0.1.4 has no per-session env-injection field, and AGENT_GIT_SKILLS is passed only to the sandbox-init exec (empty at turn time), so a reproducible frozen-tag round pins by delivering EMBER_CORPUS_REF to the guard exec; confirming that turn-time delivery end-to-end is the remaining live check.',
...
function harnessPreflight(cli: Cli, corpusTag: string, corpusCommit: string | null): HarnessConfig | null {
  ...
  // side is BUILT (the corpus guard clones the tag when EMBER_CORPUS_REF is set in its exec env —
  // AGENT_GIT_SKILLS is init-only, see frozenCorpusRefBlockers) but the turn-time delivery of that
  // env to the guard is not yet verified live, so the WIRED gate below still stands.
  ...
  //   - the boot HONOURS ref:<tag> so measurements are corpus-pinned (P3) — the guard clones the
  //     tag when EMBER_CORPUS_REF reaches its exec env, but that turn-time delivery has not been
  //     observed live, so a run could still read the default (main), an unpinned corpus;
